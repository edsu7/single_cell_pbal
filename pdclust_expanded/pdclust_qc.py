...
def plot_dendrogram_CNV(CNV,stats,annotations,annotations_category_colored,cut_tree):
    ...
    fig.layout["yaxis1"].update(showticklabels=False,ticktext=ordered_labels,tickfont=dict(size=8),tickvals=zero_vals,showticklabels=False,ticks="",range=(min(zero_vals)-3,max(zero_vals)+2.5),side='right')
...
def plot_scatter_MDS(pairwise_array,stats,annotation,annotations_category_colored):
    ...
    if isinstance(stats.loc[:,annotation].values.tolist()[0],float):
        for inst in stats.loc[pairwise_array.columns.tolist(),"pdclust_clusters"].unique().tolist():
            tmp=[i for i,val in enumerate(stats.loc[pairwise_array.columns.tolist(),"pdclust_clusters"].values.tolist()) if val==inst]
            fig.append_trace(go
                             .Scattergl(
        ...
        fig.append_trace(go
                         .Scattergl(
                             ...
                             marker=dict(symbol='square',
                                         size=20,
                                         color=[annotations_category_colored[annotation][z]['color'] for z in list(annotations_category_colored[annotation].keys())[1:]],
                                         mode='markers'
                       ),
                         ),1,1
        )
...
def merge_cpgs(cpgs):
    ...
    ro.r('smoothed_bsseq<-BSmooth(combined, h = 1000, verbose=TRUE, BPPARAM = MulticoreParam(workers = 12))')
    ro.r('rm(combined)')
...
def find_DMRs(smoothed_python_df,min_cpg_cov,min_cpg_in_window,fdr_cutoff,cpg_window):
    ...
    fig_dmr['layout']['xaxis1'].update(showticklabels=False)
